...
class Reports extends MY_Controller
{
    ...
    function b_plus_daily_report_custom() {

        $start_date = date("Y-m-d 00:00:00");
        $end_date = date("Y-m-d 23:59:59");
        $first_day_of_this_month = date("Y-m-d 00:00:00");
        $papana = date("Y-m-d 23:59:59");
        $this->data['start_date'] = $start_date;
        $this->data['end_date'] = $end_date;
        $this->data['cus_end'] = $end_date;

        if ($this->input->post('submit')) {
            $custom_date = explode('/', $this->input->post('custom_date'));
            $custom_date = $custom_date[2]."-".$custom_date[1]."-".$custom_date[0];
            $start_date = $custom_date . " 00:00:00";
            $end_date = $custom_date . " 23:59:59";
            $first_day_of_this_month = $custom_date . " 00:00:00";

            $this->data['start_date'] = $start_date;
            $this->data['end_date'] = $end_date;
            $this->data['cus_end'] = $end_date;
        }
        $this->data['cat_sale'] = $this->db->query("
            SELECT si.*, p.name, p.category_id, c.id AS cat_id, c.code, c.name,
            SUM(si.subtotal) AS sub_total, SUM(si.unit_quantity) AS qty,
            SUM(si.item_discount) AS tot_discount
            FROM sma_sales AS s
            INNER JOIN sma_sale_items AS si ON s.id=si.sale_id
            INNER JOIN sma_products AS p ON si.product_id=p.id
            INNER JOIN sma_categories AS c ON p.category_id=c.id
            WHERE s.date BETWEEN '{$start_date}' AND '{$end_date}'
            GROUP BY c.id ORDER BY c.code
        ")->result();

        $this->data['stock_report'] = $this->db->query("
            SELECT SUM(spi.quantity_balance) AS QB, (spi.quantity_balance*sp.price) AS PRODUCT_VALUE, spi.product_id, spi.quantity_balance, sc.code, sc.name
            FROM sma_purchase_items AS spi INNER JOIN sma_products AS sp
            ON sp.id=spi.product_id INNER JOIN sma_categories AS sc
            ON sc.id=sp.category_id
            -- WHERE spi.date BETWEEN '{$start_date}' AND '{$end_date}'
            GROUP BY sc.code ORDER BY sc.code
        ")->result();

        $tot_val = array();
        foreach($this->data['stock_report'] as $val) {
            $tot_val[] = $val->PRODUCT_VALUE;
        }
        $this->data['stock_amount'] = array_sum($tot_val);
        $this->data['total_vat'] = $this->db->query("SELECT SUM(order_tax) AS TOT_VAT FROM sma_sales
            WHERE `date` BETWEEN '{$start_date}' AND '{$end_date}'
        ")->row()->TOT_VAT;
        $this->data['expense_data'] = $this->db->query("
            SELECT ec.name, SUM(e.amount) AS cat_amount, e.* FROM sma_expense_categories AS ec
            LEFT JOIN sma_expenses AS e ON ec.id=e.category_id
            WHERE e.date BETWEEN '{$start_date}' AND '{$end_date}'
            GROUP BY e.category_id
        ")->result();

        // previous cash init data
        $prev_cash = $this->db->query("SELECT id, `date`, cash_in_hand FROM sma_pos_register ORDER BY id DESC LIMIT 1")->row()->cash_in_hand;
        $all_expense = $this->db->query("SELECT SUM(amount) AS exp_amount FROM sma_expenses WHERE `date` < '{$first_day_of_this_month}'")->row()->exp_amount;
        $all_sale = $this->db->query("SELECT SUM(total) AS total_sale FROM sma_sales WHERE `date` < '{$first_day_of_this_month}'")->row()->total_sale;
        $this->data['init_cash'] = ($prev_cash+$all_sale) - $all_expense;
        $all_expense = $this->db->query("SELECT SUM(amount) AS exp_amount FROM sma_expenses WHERE `date` BETWEEN '{$start_date}' AND '{$end_date}'")->row()->exp_amount;
        $all_sale = $this->db->query("SELECT SUM(total) AS total_sale FROM sma_sales WHERE `date` BETWEEN '{$start_date}' AND '{$end_date}'")->row()->total_sale;
        $this->data['prev_cash'] = ($this->data['init_cash']-$all_sale) + $all_expense;
        $prev_cash = $this->db->query("SELECT id, `date`, cash_in_hand FROM sma_pos_register ORDER BY id DESC LIMIT 1")->row()->cash_in_hand;
        $all_expense = $this->db->query("SELECT SUM(amount) AS exp_amount FROM sma_expenses WHERE `date` < '{$first_day_of_this_month}'")->row()->exp_amount;
        $all_sale = $this->db->query("SELECT SUM(total) AS total_sale FROM sma_sales WHERE `date` < '{$first_day_of_this_month}'")->row()->total_sale;

        $this->data['prev_cash'] = ($prev_cash+$all_sale) - $all_expense;
        $this->data['receive_data'] = $this->db->query("
            SELECT a.date, SUM(ai.quantity) AS adj_qty, (ai.quantity*p.price) AS RCV_PRODUCT_VALUE, c.name AS cat_name FROM sma_adjustments AS a
            INNER JOIN sma_adjustment_items AS ai ON a.id=ai.adjustment_id
            INNER JOIN sma_products AS p ON ai.product_id=p.id
            INNER JOIN sma_categories AS c ON c.id=p.category_id
            WHERE a.date BETWEEN '{$start_date}' AND '{$end_date}'
            GROUP BY c.code
        ")->result();
        $this->data['custom_stock_report'] = $this->db->query("
            SELECT SUM(spi.quantity_balance) AS QB, (spi.quantity_balance*sp.price) AS PRODUCT_VALUE, spi.product_id, spi.quantity_balance, sc.code, sc.name
            FROM sma_purchase_items AS spi INNER JOIN sma_products AS sp
            ON sp.id=spi.product_id INNER JOIN sma_categories AS sc
            ON sc.id=sp.category_id GROUP BY sc.code ORDER BY sc.code
        ")->result();
        // new -- edited data format
        $cus_end = date("Y-m-d 23:59:59");
        $this->data['custom_receive_data'] = $this->db->query("
            SELECT a.date, SUM(ai.quantity) AS adj_qty, c.name AS cat_name FROM sma_adjustments AS a
            INNER JOIN sma_adjustment_items AS ai ON a.id=ai.adjustment_id
            INNER JOIN sma_products AS p ON ai.product_id=p.id
            INNER JOIN sma_categories AS c ON c.id=p.category_id
            WHERE a.date BETWEEN '{$start_date}' AND '{$cus_end}'
            GROUP BY c.code
        ")->result();
        $this->data['custom_cat_sale'] = $this->db->query("
            SELECT si.*, p.name, p.category_id, c.id AS cat_id, c.code, c.name,
            SUM(si.subtotal) AS sub_total, SUM(si.unit_quantity) AS qty,
            SUM(si.item_discount) AS tot_discount
            FROM sma_sales AS s
            ...
            INNER JOIN sma_products AS p ON si.product_id=p.id
            INNER JOIN sma_categories AS c ON p.category_id=c.id
            WHERE s.date BETWEEN '{$start_date}' AND '{$cus_end}'
            GROUP BY c.id ORDER BY c.code
        ")->result();

        // end new -- edited data format
        foreach($this->data['custom_stock_report'] as $cs) {
            foreach($this->data['custom_receive_data'] as $rcv_data) {
        ...
        foreach($this->data['custom_stock_report'] as $cs) {
            foreach($this->data['custom_cat_sale'] as $cat_data) {
        ...
        $meta = array('page_title' => 'B+ Plus Report');
        $this->page_construct('reports/b_plus_daily_report_custom', $meta, $this->data);
    }
    ...
    function b_plus_report() {
        ...
        $this->data['month'] = date("F", strtotime("last month"));
        ...
        $this->data['total_vat'] = $this->db->query("
            SELECT SUM(order_tax) AS TOT_VAT FROM sma_sales
            WHERE `date` BETWEEN '{$start_date}' AND '{$end_date}'
        ")->row()->TOT_VAT;
        ...
        // previous cash init data
        $first_day_of_this_month = date("Y-m-d 00:00:00", strtotime("first day of this month"));
        $prev_cash = $this->db->query("SELECT id, `date`, cash_in_hand FROM sma_pos_register ORDER BY id DESC LIMIT 1")->row()->cash_in_hand;
        $all_expense = $this->db->query("SELECT SUM(amount) AS exp_amount FROM sma_expenses WHERE `date` < '{$first_day_of_this_month}'")->row()->exp_amount;
        $all_sale = $this->db->query("SELECT SUM(total) AS total_sale FROM sma_sales WHERE `date` < '{$first_day_of_this_month}'")->row()->total_sale;
        $this->data['init_cash'] = ($prev_cash+$all_sale) - $all_expense;
        // dd($this->data['prev_cash']);
        ...
        /*echo "<pre>";
        dd($this->data['custom_stock_report']);*/

        /*$this->data['papana_stock_data'] = $this->data['custom_stock_report'];
        foreach($this->data['papana_stock_data'] as $cs) {
            foreach($this->data['cat_sale'] as $cat_data) {
                if($cs->code == $cat_data->code) {
                    $cs->QB -= $cat_data->qty;
                }
            }
        ...
        foreach($this->data['papana_stock_data'] as $cs) {
            foreach($this->data['receive_data'] as $rcv_data) {
                if($cs->code == $rcv_data->cat_name) {
                    $cs->QB += $rcv_data->adj_qty;
                }
            }
        ...
        /*echo "<pre>";
        dd($this->data['papana_stock_data']);*/

        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('products_report')));
        $meta = array('page_title' => 'B+ Plus Report');
        $this->page_construct('reports/b_plus_report', $meta, $this->data);
    }
    
    function b_plus_daily_report() {
        $start_date = date("Y-m-d 00:00:00");
        $end_date = date("Y-m-d 23:59:59");
        $this->data['month'] = date("F", strtotime("last month"));
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('products_report')));
        $meta = array('page_title' => 'B+ Plus Report');
        // $this->page_construct('reports/b_plus_daily_report', $meta, $this->data);
        $this->page_construct('reports/aa', $meta, $this->data);
    }
