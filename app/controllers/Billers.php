...
class Billers extends MY_Controller
{
    ...
    function table_list($action = NULL) {
        $this->sma->checkPermissions();

        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['action'] = $action;
        $this->data['get_record'] = $this->db->query("SELECT * FROM sma_table")->result();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('tables')));
        $meta = array('page_title' => lang('tables'), 'bc' => $bc);
        $this->page_construct('billers/table_list', $meta, $this->data);
    }
    ...
    function create_table() {
        ...
        $this->data['title'] = "Create Table";
        // $this->form_validation->set_rules('table_name', 'table name', 'required|trim|xss_clean');
        // $this->form_validation->set_rules('table_desc', 'table description', 'trim|xss_clean');
        ...
        if ($this->input->post('submit')) {
            ...
            #insert data
            $sql = "
            INSERT INTO sma_table
            (table_name, table_desc)
            VALUES
            ( ?, ? )";
            $this->db->query($sql, array($table_name, $table_desc));

            $this->session->set_flashdata('message', 'table name created successfully');
            redirect("billers/table_list");

        } else {

            $bc = array(array('link' => site_url('home'), 'page' => lang('home')), array('link' => site_url('billers/table_list'), 'page' => lang('table_list')), array('link' => '#', 'page' => lang('create_user')));
            $meta = array('page_title' => lang('create_table'), 'bc' => $bc);
            $this->page_construct('billers/create_table', $meta, $this->data);
        }
    }

    function table_delete($id) {
        $sql = $this->db->query("DELETE FROM sma_table WHERE table_id='".$id."'");
        if($sql) {
    ...
    function waiter_list($action = NULL) {
        $this->sma->checkPermissions();

        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['action'] = $action;
        $this->data['get_record'] = $this->db->query("SELECT * FROM sma_waiter")->result();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('waiter_list')));
        $meta = array('page_title' => lang('waiter_list'), 'bc' => $bc);
        $this->page_construct('billers/waiter_list', $meta, $this->data);
    }

    function create_waiter() {
        if (!$this->Owner) {
            $this->session->set_flashdata('warning', lang("access_denied"));
            redirect($_SERVER["HTTP_REFERER"]);
        }

        $this->data['title'] = "Create Waiter";
        // $this->form_validation->set_rules('table_name', 'table name', 'required|trim|xss_clean');
        // $this->form_validation->set_rules('table_desc', 'table description', 'trim|xss_clean');
        ...
        if ($this->input->post('submit')) {
            ...
            $waiter_desc = $this->input->post('waiter_desc');
            
            #insert data
            $sql2 = "
            INSERT INTO sma_waiter
            (waiter_name, waiter_desc)
            VALUES
            ( ?, ? )";
            $this->db->query($sql2, array($waiter_name, $waiter_desc));

            $this->session->set_flashdata('message', 'waiter name created successfully');
            redirect("billers/waiter_list");

        } else {

            $bc = array(array('link' => site_url('home'), 'page' => lang('home')), array('link' => site_url('billers/waiter_list'), 'page' => lang('waiter_list')), array('link' => '#', 'page' => lang('create_user')));
            $meta = array('page_title' => lang('create_waiter'), 'bc' => $bc);
            $this->page_construct('billers/create_waiter', $meta, $this->data);
        }
    }

    function waiter_delete($id) {
        $sql = $this->db->query("DELETE FROM sma_waiter WHERE waiter_id='".$id."'");
        if($sql) {
            $this->session->set_flashdata('message', 'waiter deleted successfully');
            redirect("billers/waiter_list");
        }
    }
