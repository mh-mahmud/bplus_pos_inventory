...
class Pos extends MY_Controller
{
    ...
    /*public function index($sid = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE && !empty($products) && !empty($data)) {
            ...
            ")->row()->TOT_VAT;
            $expense_data = $this->db->query("
                SELECT SUM(amount) AS EXPENSE FROM sma_expenses
                WHERE `date` BETWEEN '{$first_day}' AND '{$last_day}'
            ")->row()->EXPENSE;
            $total_sale = $this->db->query("
                SELECT SUM(total) AS SALE FROM sma_sales
                WHERE `date` BETWEEN '{$first_day}' AND '{$last_day}'
            ")->row()->SALE;
            $previous_cash = $this->db->query("SELECT cash_in_hand FROM sma_pos_register ORDER BY id DESC LIMIT 1")->row()->cash_in_hand;
            $this->data['cash'] = ($total_sale+$previous_cash) - $expense_data;
    ...
    public function index($sid = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE && !empty($products) && !empty($data)) {
            ...
            $all_expense = $this->db->query("SELECT SUM(amount) AS exp_amount FROM sma_expenses WHERE `date` < '{$start_date}'")->row()->exp_amount;
            $all_sale = $this->db->query("SELECT SUM(total) AS total_sale FROM sma_sales WHERE `date` < '{$start_date}'")->row()->total_sale;
            $this->data['prev_cash'] = ($prev_cash+$all_sale) - $all_expense;
            $this->data['expense_data'] = $this->db->query("
                SELECT SUM(amount) AS amount FROM sma_expenses WHERE `date` BETWEEN '{$start_date}' AND '{$end_date}'
            ")->row()->amount;
            $this->data['total_sale_data'] = $this->db->query("SELECT SUM(total) AS subtotal FROM sma_sales WHERE `date` BETWEEN '{$start_date}' AND '{$end_date}'")->row()->subtotal;
            $this->data['running_cash'] = ($this->data['prev_cash'] + $this->data['total_sale_data']) - $this->data['expense_data'];
            $this->load->view($this->theme . 'pos/add', $this->data);
        }
    }
    ...
    public function view($sale_id = NULL, $modal = NULL)
    {
		...
		// $this->data['waiter_name'] = $_POST['waiter_name'];
