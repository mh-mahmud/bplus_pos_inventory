...
class Welcome extends MY_Controller
{
    ...
    public function stock_adjustment() {
        $stock_data = $this->db->query("SELECT id, product_id, product_code, sale_id, net_unit_price, quantity, unit_quantity, tax FROM sma_sale_items WHERE quantity != unit_quantity")->result();
        foreach($stock_data as $val) {
            $val->tax = $val->unit_quantity - $val->quantity;
        }
        // product less
        foreach($stock_data as $v) {
            $this->db->query("UPDATE sma_products SET quantity = quantity + '{$v->tax}' WHERE id='{$v->product_id}'");
            $this->db->query("UPDATE sma_purchase_items SET quantity_balance = quantity_balance+'{$v->tax}' WHERE product_id='{$v->product_id}'");
            $this->db->query("UPDATE sma_warehouses_products SET quantity=quantity-'{$v->tax}' WHERE product_id='{$v->product_id}'");
            $this->db->query("UPDATE sma_sale_items SET quantity=quantity+'{$v->tax}' WHERE id='{$v->id}'");
            $this->db->query("UPDATE sma_sales SET total_items=total_items+'{$v->tax}' WHERE id='{$v->sale_id}'");
